...
import com.atguigu.education_online.member.bean.{DwsMember, MemberZipper, MemberZipperResult}
import com.atguigu.education_online.member.dao.DwdMemberDao
...
object DwsMemberService {
  ...
  def importMemberUseApi(sparkSession: SparkSession, dt: String): Unit = {
    ...
    // todo dwdMember.join(dwdMemberRegtype, dwdMember("uid") === dwdMemberRegtype("uid") && dwdMember("dn") === dwdMemberRegtype("dn"))
    // todo 此方式不会去重，产生两个 uid 和两个 dn 会报错

    // todo Seq会去重，后面select不会报错
    ...
    val result =
      ...
      .select("uid", "ad_id", "fullname", "iconurl", "lastlogin", "mailaddr", "memberlevel", "password"
        ...
        , "vip_operator", "dt", "dn")
      .as[DwsMember]//.cache()    todo   DF和DS默认缓存 32.8M  内存
        .persist(StorageLevel.MEMORY_AND_DISK_SER)   // todo   DF和DS  32.1M  内存

        //todo 推荐使用DF和DS 比rdd缓存优化（rdd需要手动注册），默认使用kryo序列化优化

    //.rdd.cache()           //  todo java序列化 1760M内存
      //.rdd.persist(StorageLevel.MEMORY_AND_DISK_SER) // todo 修改了了缓存级别配合kryo序列化缓存 200M 内存

    result.foreach(item => println(item.paymoney))
    ...
    val resultData = result.groupByKey(item => item.uid + "_" + item.dn)
      .mapGroups { case (key, iters) =>
        ...
        val siteid = dwsMembers.map(_.siteid).headOption.getOrElse("")
        ...
        DwsMember(uid, ad_id, fullname, icounurl, lastlogin, mailaddr, memberlevel, password, paymoney,
...
//    }
  ...
  def importMember(sparkSession: SparkSession, time: String): Unit = {
    ...
    sparkSession.sql("select uid,first(ad_id),first(fullname),first(iconurl),first(lastlogin)," +
      ...
      "group by uid,dn,dt")
      .coalesce(1)  // todo OOM 可以使用repartition()算子替换
    ...
    //查询历史拉链表数据
    val historyResult: Dataset[MemberZipper] = sparkSession.sql("select * from dws.dws_member_zipper")
      .as[MemberZipper]
    ...
    dayResult.union(historyResult).groupByKey(item => item.uid + "_" + item.dn)
      ...
      .coalesce(3)
      .write.mode(SaveMode.Overwrite)
      .insertInto("dws.dws_member_zipper") //重组对象打散 刷新拉链表
