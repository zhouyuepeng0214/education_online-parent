...
import com.atguigu.education_online.qz.bean._
...
object EtlDataService {
  ...
  def etlQzMemberPaperQuestion(ssc: SparkContext, sparkSession: SparkSession) = {
    ...
    ssc.textFile("hdfs://hadoop110:9000/user/atguigu/ods/QzMemberPaperQuestion.log").filter(item => {
    ...
    sparkSession.sql("select * from dwd.dwd_qz_member_paper_question limit 5").show()
  }
  ...
  def etlQzQuestionType(ssc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    ssc.textFile("hdfs://hadoop110:9000/user/atguigu/ods/QzQuestionType.log").filter(item => {
      val obj = ParseJsonData.getJsonData(item)
      obj.isInstanceOf[JSONObject]
    }).mapPartitions(partitions => {
      partitions.map(item => {
    ...
    sparkSession.sql("select * from dwd.dwd_qz_question_type limit 5").show()
  }
  ...
  def etlQzQuestion(ssc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    ssc.textFile("hdfs://hadoop110:9000/user/atguigu/ods/QzQuestion.log").filter(item => {
      val obj = ParseJsonData.getJsonData(item)
      obj.isInstanceOf[JSONObject]
    }).mapPartitions(partitions => {
      partitions.map(item => {
    ...
    sparkSession.sql("select * from dwd.dwd_qz_question limit 5").show()
  }
  ...
  def etlQzCenter(ssc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    ssc.textFile("hdfs://hadoop110:9000/user/atguigu/ods/QzCenter.log").filter(item => {
      val obj = ParseJsonData.getJsonData(item)
      obj.isInstanceOf[JSONObject]
    ...
    sparkSession.sql("select * from dwd.dwd_qz_center limit 5").show()
  }
  ...
  def etlQzPaper(ssc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    ssc.textFile("hdfs://hadoop110:9000/user/atguigu/ods/QzPaper.log").filter(item => {
      val obj = ParseJsonData.getJsonData(item)
      obj.isInstanceOf[JSONObject]
    }).mapPartitions(partitions => {
      partitions.map(item => {
    ...
    sparkSession.sql("select * from dwd.dwd_qz_paper limit 5").show()
  }
  ...
  def etlQzCenterPaper(ssc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    ssc.textFile("hdfs://hadoop110:9000/user/atguigu/ods/QzCenterPaper.log").filter(item => {
      val obj = ParseJsonData.getJsonData(item)
      obj.isInstanceOf[JSONObject]
    }).mapPartitions(partitions => {
      partitions.map(item => {
    ...
    sparkSession.sql("select * from dwd.dwd_qz_center_paper limit 5").show()
  }
  ...
  def etlQzPaperView(ssc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    ssc.textFile("hdfs://hadoop110:9000/user/atguigu/ods/QzPaperView.log").filter(item => {
      val obj = ParseJsonData.getJsonData(item)
      obj.isInstanceOf[JSONObject]
    }).mapPartitions(partitions => {
      partitions.map(item => {
    ...
    sparkSession.sql("select * from dwd.dwd_qz_paper_view limit 5").show()
  }
  ...
  def etlQzBusiness(ssc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    ssc.textFile("hdfs://hadoop110:9000/user/atguigu/ods/QzBusiness.log").filter(item => {
      val obj = ParseJsonData.getJsonData(item)
      obj.isInstanceOf[JSONObject]
    }).mapPartitions(partitions => {
      partitions.map(item => {
    ...
    sparkSession.sql("select * from dwd.dwd_qz_business limit 5").show()
  }
  ...
  def etlQzMajor(ssc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    ssc.textFile("hdfs://hadoop110:9000/user/atguigu/ods/QzMajor.log").filter(item => {
      val obj = ParseJsonData.getJsonData(item)
      obj.isInstanceOf[JSONObject]
    }).mapPartitions(partitions => {
      partitions.map(item => {
    ...
    sparkSession.sql("select * from dwd.dwd_qz_major limit 5").show()
  }
  ...
  def etlQzWebsite(ssc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    ssc.textFile("hdfs://hadoop110:9000/user/atguigu/ods/QzWebsite.log").filter(item => {
      val obj = ParseJsonData.getJsonData(item)
      obj.isInstanceOf[JSONObject]
    }).mapPartitions(partitions => {
      partitions.map(item => {
    ...
    sparkSession.sql("select * from dwd.dwd_qz_website limit 5").show()
  }
  ...
  def etlQzCourseEdusubject(ssc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    ssc.textFile("hdfs://hadoop110:9000/user/atguigu/ods/QzCourseEduSubject.log").filter(item => {
      val obj = ParseJsonData.getJsonData(item)
      obj.isInstanceOf[JSONObject]
    }).mapPartitions(partitions => {
      partitions.map(item => {
    ...
    sparkSession.sql("select * from dwd.dwd_qz_course_edusubject limit 5").show()
  }
  ...
  def etlQzCourse(ssc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    ssc.textFile("hdfs://hadoop110:9000/user/atguigu/ods/QzCourse.log").filter(item => {
      val obj = ParseJsonData.getJsonData(item)
      obj.isInstanceOf[JSONObject]
    }).mapPartitions(partitions => {
      partitions.map(item => {
        ...
        val sequence = jsonObject.getString("sequece")
    ...
    sparkSession.sql("select * from dwd.dwd_qz_course limit 5").show()
  }
  ...
  def etlQzSiteCourse(ssc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    ssc.textFile("hdfs://hadoop110:9000/user/atguigu/ods/QzSiteCourse.log").filter(item => {
      val obj = ParseJsonData.getJsonData(item)
      obj.isInstanceOf[JSONObject]
    }).mapPartitions(partitions => {
      partitions.map(item => {
        ...
        val creator = jsonObject.getString("creator")
        val createtime = jsonObject.getString("createtime")
        val helppaperstatus = jsonObject.getString("helpparperstatus")
    ...
    sparkSession.sql("select * from dwd.dwd_qz_site_course limit 5").show()
  }
  ...
  def etlQzPointQuestion(ssc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    ssc.textFile("hdfs://hadoop110:9000/user/atguigu/ods/QzPointQuestion.log")
      .filter(item => {
      ...
      .mapPartitions(partition => {
        partition.map(item => {
          val nObject: JSONObject = ParseJsonData.getJsonData(item)
          val pointid = nObject.getIntValue("pointid")
          val questionid = nObject.getIntValue("questionid")
          val questtype = nObject.getIntValue("questtype")
          val creator = nObject.getString("creator")
          val createtime = nObject.getString("createtime")
          val dt = nObject.getString("dt")
    ...
    sparkSession.sql("select * from dwd.dwd_qz_point_question limit 5").show()
  ...
  def etlQzPoint(ssc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    ssc.textFile("hdfs://hadoop110:9000/user/atguigu/ods/QzPoint.log").filter(item => {
      val obj = ParseJsonData.getJsonData(item)
      obj.isInstanceOf[JSONObject]
    }).mapPartitions(partitions => {
      partitions.map(item => {
    ...
    sparkSession.sql("select * from dwd.dwd_qz_point limit 5").show()
  }
  ...
  def etlQzChapterList(ssc: SparkContext, sparkSession: SparkSession): Unit = {
    import sparkSession.implicits._
    ssc.textFile("hdfs://hadoop110:9000/user/atguigu/ods/QzChapterList.log")
      .filter(item => {
        val nObject: JSONObject = ParseJsonData.getJsonData(item)
        nObject.isInstanceOf[JSONObject]
      }).mapPartitions(partition => {
      partition.map(item => {
        JSON.parseObject(item, classOf[DwdQzChapterList])
      })
    })
      .toDF().coalesce(1)
      .write.mode(SaveMode.Append)
      .insertInto("dwd.dwd_qz_chapter_list")
    sparkSession.sql("select * from dwd.dwd_qz_chapter_list limit 5").show()
  }
  ...
  def etlQzChapter(ssc: SparkContext, sparkSession: SparkSession): Unit = {
    import sparkSession.implicits._
    ssc.textFile("hdfs://hadoop110:9000/user/atguigu/ods/QzChapter.log")
      .filter(item => {
        val nObject: JSONObject = ParseJsonData.getJsonData(item)
        nObject.isInstanceOf[JSONObject]
      })
      .mapPartitions(partition => {
        partition.map(item => {
          JSON.parseObject(item, classOf[DwdQzChapter])
        })
      })
      .toDF().coalesce(1)
      .write.mode(SaveMode.Append)
      .insertInto("dwd.dwd_qz_chapter")
    sparkSession.sql("select * from dwd.dwd_qz_chapter limit 5").show()
